...
def univec(a, b, c, d, x0, index_1, index_2, index_3, index_4):                # build the matrix B in x0
    B = np.zeros((2, 3))

    if index_3 == index_1:                                                     # checks to create the matrix B properly according to which feature is common to both the couples
        B[0][0] = a/x0[index_1]
        B[0][1] = b/x0[index_2]
        B[0][2] = 0
...
def delta(lista, Data, i):                                                     # to pick random points in the interval [x_min, m_max]
    ...
    x_3_min = np.min(Data[lista[i][1][0]])
    x_3_max = np.max(Data[lista[i][1][0]])
    x_4_min = np.min(Data[lista[i][1][1]])
    x_4_max = np.max(Data[lista[i][1][1]])

    return(x_1_min, x_1_max, x_2_min, x_2_max, x_3_min, x_3_max, x_4_min,
           x_4_max)


l = list(combinations(list(other_columns), 2))                                 # get all possible combinations of couples of the features
    ...
    for j in range(repeat):
        ...
        for k in range(N):
            ...
            index_3 = pd.DataFrame(tx, columns = 
                                   other_columns).columns.get_loc(
                                       lista[i][1][0])                         # get index of the third feature
            index_4 = pd.DataFrame(tx, columns = 
                                   other_columns).columns.get_loc(
                                       lista[i][1][1])                         # get index of the fourth feature
            ...
            def MAIN(beta):
                ...
                if index_3 == index_1:
                    der = np.array([grad1, grad2, grad4]) 
                elif index_3 == index_2:
                    der = np.array([grad1, grad2, grad4])
                elif index_4 == index_2:
                    der = np.array([grad1, grad3, grad4])                      # create derivative vector which has to be parallel to the orthogonal vector un in x0
...
array = np.array([np.mean(A, axis = 1), 
                ...
                np.std(C, axis = 1),
                np.mean(D, axis = 1),
                np.std(D, axis = 1)]).transpose()                              # create "summary" containing mean and std of the results
